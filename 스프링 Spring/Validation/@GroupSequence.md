...
그런 경우 @GroupSequence를 사용해주면 된다. @GroupSequence는 검증 어노테이션을 그룹으로 묶어서 각 그룹의 순서를 지정해줄 수 있도록 한다.
...
그리고 @GroupSequence를 사용하여 원하는 순서대로 정리해준다.
...
Controller에서 @Valid가 있었던 부분을 @Validated로 바꾸어준다.

@Validated는 @Valid의 동작을 대체하면서, 순서 정의 기능을 추가해주기 때문에 @Valid를 완전히 지워줘도 괜찮다.
