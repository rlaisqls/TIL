...
- 리누스 토발즈가 커널 개발 / 커널: 소스 공개, 누구나 수정 및 패키징하여 자유롭게 배포 가능
...
1. 슬랙웨어
   - SuSE: 독일, YaST 유틸리티 제공
   - Vector Linux
2. 데비안(Debian): 이안 머독이 설립, 프로젝트에 참여했던 개인들에 의해 패키지화, 패키지 관리 도구(dpkg -> apt /
설치나 업데이트 시 다른 패키지와의 의존성 확인과 보안 관련 업데이트 자동 수행)
   - Knoppix
   - Corel
   - Ubuntu: 데비안 GNU/Linux 기반 운영체제 / 그놈(GNOME) 데스크톱 환경 기반의 그래픽 셸인 유니티(Unity) 사용 / 영국 캐노티컬
   - Linux Mint
   - Elementary OS
   - 백트랙(BackTrack)
   - Kali Linux: Offensive Security에서 만든 Debian 기반 리눅스 / 정보보안 테스트를 위해 해킹 관련 도구, 설명서 내장
3. 레드햇: 가장 인기 있는 리눅스 배포판 / 패키지 관리 도구(RPM, YUM)
   - RHEL: 유료 배포판
   - CentOS
   - Fedora
   - Oracle Linux
4. 아시아눅스: 한국(한글과 컴퓨터) + 일본, 중국(홍기 리눅스), 스리랑카(엔터프라이즈 테크놀로지) 등의 아시아 기업과
협력하여 개발
* 순서: SLS -> 슬랙웨어 -> Debian -> SuSE, 레드햇 -> CentOS -> Ubuntu
